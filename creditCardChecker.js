...
const validateCred = (array) => {
  ...
  for (let i = 0; i < reversedArray.length; i++) {
    ...
    if (i === 0 || Number.isInteger(i / 2)) {
      ...
      let doubledNumber = reversedArray[i] * 2
      if (doubledNumber > 9) {
        doubledNumber = doubledNumber - 9
      }
      reversedArray[i] = doubledNumber
      sum += reversedArray[i]
    }
  }
...
const findInvalidCards = (nestedArray) => {
  ...
  for (const oneArray of nestedArray) {
    const isValid = validateCred(oneArray)
    if (!isValid) {
      invalidCards.push(oneArray)
    }
  }
  return invalidCards
}
...
const idInvalidCardCompanies = (nestedInvalidCardsArray) => {
  ...
  for (const oneArray of nestedInvalidCardsArray) {
